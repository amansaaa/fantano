...
import type { Artist, EndorsedTrack, Review, SimilarArtist } from "@/lib/types";
...
SELECT reviews.video_id, videos.title AS video_title,
       COALESCE(releases.title, tracks.title) AS title, releases.kind, reviews.score_text
FROM reviews
...
/**
 * this artist's reviews, newest first, each with its fav tracks. empty for artists he never
 * reviewed (they're only credited on other people's songs).
 *
 *   await getReviews(239)
 *   // -> [{ title: "petal", score_text: "5/10", fav_tracks: ["...", ...], ... }]
 */
...
//                "Why Sheck Wes: has a song with Travis Scott, credited in {video}" line
// ranked by number of videos, then the newest video.
// every link today is a written "ft." credit (label collaborator, no start_s), so the label and
// start_s tie-breaks never decide anything yet. they stay so the rule works for any label
// schema.sql allows
...
/** every artist who shares a song credit with this one, ranked for the grid. */
...
SELECT tracks.id AS track_id, tracks.title, tracks.cover_url, tracks.artist_id,
       endorsements.source, endorsements.video_id, videos.title AS video_title,
...
ORDER BY videos.published_at DESC, endorsements.id
`;
